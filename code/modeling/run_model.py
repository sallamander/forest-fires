...
def own_grid_search(model_name, train_data, test_data, train_data2): 
	...
	for idx, param_comb in enumerate(param_combs): 
		...
		for months_forward in xrange(0, 132, 2): 
			...
			training_set, validation_set = tt_split_early_late(train, date_split, months_forward, months_backward=0.25, days_forward=2, weeks_forward=months_forward)
			# for months_forward in xrange(0, 33, 1): 
			'''
			training_set, validation_set = tt_split_same_months(train, 2013, 1, days_back=15)	
	...
	roc_save_filename = './model_output/roc_auc_quartermonthprior_15_' + model_name
...
if __name__ == '__main__': 
	...
	train2, test2 = tt_split_same_months(train, 2012, [1], days_back=14, exact_split_date=test.date_fire.max(), direct_prior_days=False)
	...
	filename = './model_output/' + model_name + '_preds_probs_twoweeksprioryear_15.csv'
