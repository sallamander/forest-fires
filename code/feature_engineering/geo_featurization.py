...
This module uses a driver function in conjunction with multiprocessing to find 
observations in a DataFrame that are within a certain distance and time period 
from a given observation. Distance is defined as plus/minus some distance in 
lat/long space, and a time interval is given as anything that `datetime.timedelta` 
allows (minutes, hours, weeks, days, etc.). It also allows for the opportunity 
to ask if these nearby observations are forest-fires or not.  

The only function that is meant to be called externally in this module is the 
driver function - `gen_nearby_fires_count`. In this function, there is a use 
of the kwargs argument in a somewhat non-traditional way. This has to do 
with how the `create_inputs.py` module calls this function. To allow for 
flexibility in the function calls in the `create_inputs.py` module, a somewhat 
non-traditional use of kwargs helped. 
"""
...
def gen_nearby_fires_count(df, kwargs):
    ...
    For each row in the detected fires data set, create a bunch of new columns: 
        
        * One holding the count of nearby detected fires (regardless of whether 
          they are labeled as a forest fire), going back 1-7 days 
        * One holding the count of nearby detected fires that are labeled as 
          forest-fires (fire_bool = True), going back 1-7 days
        * One holding the count of nearby detected fires that are labeled as 
          forest-fires (fire_bool = True), going back 1, 2, and 3 years 

    Nearby will be denoted by +/- some distance in lat/long space. 

    This function will just act as a driver for this entire process, and as 
    the function that is meant to be called external from this module. It will 
    drive by calling all of the other helper functions in this module. It will 
    also multiprocess this across all available cores be default. 

    Args: 
    ----
        df: Pandas DataFrame 
        kwargs: dct
            Holds arguments to use in the function. Here we expect the 
            'time_measures' and 'dist_measure' to keywords to be passed in. See 
            the module docstring for an explanation of the use of kwargs variable 
            here. 
...
def _prep_multiprocessing(df): 
    ...
    For multiprocessing, the df needs to be as lightweight as possible. This 
    means dropping any duplicate observations in terms of lat/long coordinates 
    and date. We also don't want to count any duplicate observations like this 
    towards the overall count of nearby fires/obs. This is the first step in 
    this function. 

    The second step involves prepping everything for multiprocessing. The
    quickest/most efficient method found to multiprocess this was the following: 

        * Create a percentile column in the DF corresponding to date
        * Create a dictionary lookup for all rows in a corresponding date 
          percentile. This allows for smarter, more efficient querying 
          (e.g. it's only necessary to query the nearby percentiles, and 
          not every one). 
    ...
    ------
        multiprocessing_df: Pandas DataFrame
            Holds the modified DataFrame, with duplicates dropped and the date 
            percentile column added. 
        dt_percentiles_df_dict: dct of DataFrames 
            Holds the dictionary lookup for all rows in a corresponding date 
            percentile. The key corresponds to the date percentile, and the 
            value corresponds to a DataFrame of all rows in that date percentile. 
    """
...
def _handle_date_percentiles(df): 
    ...
    percentiles by date, and add a column holding what percentile each row is 
    in. In addition, output a dictionary that holds 100 key-value pairs, where 
    the key is the percentile and the value is a DataFrame holding only the 
    observations in that percentile.

    This is a helper function called from `_prep_multiprocessing`, and itself 
    calls helper functions `_setup_percentiles_column` and `_setup_pctiles_df_dct`. 
    ...
    ------
        df: Pandas DataFrame
            Holds the modified DataFrame, with the date percentile column added. 
        dt_percentiles_df_dict: dct of DataFrames 
            Holds the dictionary lookup for all rows in a corresponding date 
            percentile. The key corresponds to the date percentile, and the value 
            corresponds to a DataFrame of all rows in that date percentile. 
    """
...
def _setup_pctiles_column(df, step_size, n_quantiles, new_col_name): 
    ...
    For each row in the dataframe, figure out what percentile of the date column 
    it is in, and input that into the 'date_fire_percentiles' column of the 
    dataframe. 
    ...
    ----
        df: Pandas DataFrame
        step_size: int
            Holds the number of observations to step through by to get to the 
            next percentile/bin. 
        n_quantiles: int
...
def _setup_pctiles_df_dct(df, n_quantiles, new_col_name): 
    ...
    corresponding to percentile of the date column an ob. is in, and the values 
    are Pandas DataFrames holding all the obs. in that percentile. 
...
def _grab_col_indices(df, col_list):
    ...
    This is a helper function called from `gen_nearby_fires_count`. It allows 
    the passing of integers to reference the correct "columns" of the numpy 
    array passed during multiprocessing. 
...
def query_for_nearby_fires(dt_percentiles_df_dict, dist_measure, time_measure, 
    ...
    For the inputted row, query the dataframe for the number of 'detected fires' 
    (i.e. other rows) that are within dist_measure of the inputted row 
    (lat/long wise) and within the time_measure of the inputted row (date wise). 
    Also query for the number of actual fires (i.e. other rows with 
    fire_bool = True) within the dist_measure and time_measure.

    This is a helper function called and multiprocessed from `gen_nearby_fires_count`. 

    Args: 
    ----
        dt_percentiles_df_dict: dct of DataFrames 
            Holds the key-value pairs corresponding to date percentile and rows of 
            a DataFrame in that percentile. 
        dist_measure: float
    ...
    ------
        output_dict: dct
            Holds key-value pairs for all of the info. that was queried for. 
    """
    ...
    # on the day of; we have to wait until after business hours when the fire 
    # perimter boundaries are posted. 
    ...
    # If we're doing 1-7 days, we're going to query the date percentile that the 
    # given row is in, as well as the above and below. If we're going more days 
    # back, it'll be years back, and we'll want to add in all percentiles below 
    # (not just the one directly below). 
    min_dt_percentile = 1 if time_measure > 7 else max(row_dt_percentile - 1, 1)
...
def _get_date_range(time_measure, dt):
    ...
    ----
        time_measure: int
            Holds how many days to go back in time to look for nearby_fires. 
...
def _merge_results(df, nearby_count_dicts): 
    ...
    Take in the nearby_count_dict, and merge it into the current df. The 
    nearby_count_dict holds the information calcalated in the 
    query_for_nearby_fires function for each latitude, longitude, and date combo. 
...
def calc_perc_fires(df, time_measures):
    """Calculate the percentage of nearby observations that are actually forest-fires

    Args: 
    ----
        df: pandas DataFrame
        time_measures: list of ints 
            Holds the list of days back that were used to create columns of 
            the counts of nearby observations and nearby forest-fires 
