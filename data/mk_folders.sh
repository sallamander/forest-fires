...
do 
    mkdir -p zipped_files/boundary_files/$folder_name

    if [ "$folder_name" = "fire_perimeters" ]  
    then
        for year in "${fire_tbl_years[@]}"
        do 
            mkdir -p unzipped_files/boundary_files/$folder_name/$year
        done 
    else 
...
do 
    mkdir -p zipped_files/detected_fires/$folder_name

    for year in "${fire_tbl_years[@]}"
    do 
        mkdir -p unzipped_files/detected_fires/$folder_name/$year
    done 
done 
